...
void sieve() {
	vector<int> primes;
	bool ar[90000000] //bool uses 1byte and int uses 4bytes in cpp, so when time is tight use bool
	
	int maxN = 90000000;

	ar[0] = ar[1] = true; //Not prime numbers

	for(int i = 2; i * i < maxN; i++){
		if(!ar[i]){ //if number is prime that mark its multiples as not prime
