...
void solve(){
	ll n; cin >> n;
	string revStr; cin >> revStr;
	string s = revStr;
	reverse(s.begin(), s.end());
	char c = '1';
	ll lastOccurStr = s.find_last_of(c);
	if(lastOccurStr != string::npos){
		ll lastOccurRevStr = revStr.find_last_of(c);
		ll maxRooms = max(lastOccurStr + 1, lastOccurRevStr + 1);
		cout << maxRooms * 2 << ln;
		return;
	}
	cout << n << ln;
